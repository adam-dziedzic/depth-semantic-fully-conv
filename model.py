...
class ResidualBlock(nn.Module):
    def __init__(self, in_channels, d1, d2, skip=False, stride=1):
        ...
        if not self.skip:
            self.conv4 = nn.Conv2d(in_channels, d2, 1, stride=stride,
                                   bias=False)
...
class UpProj_Block(nn.Module):
    ...
    def prepare_indices(self, before, row, col, after, dims):
        ...
        linear_indices = x_3 + dims[3] * x_2 + 2 * dims[2] * dims[3] * x_0 * 2 * \
                         dims[1] + 2 * dims[2] * dims[3] * x_1
    ...
    def unpool_as_conv(self, x, BN=True, ReLU=True, id=1):
        ...
        size_ = A_linear_indices.size()[0] + B_linear_indices.size()[0] + \
                C_linear_indices.size()[0] + D_linear_indices.size()[0]
        ...
        Y_flat.scatter_(0,
                        A_linear_indices.type(long_dtype).squeeze(),
                        A_flat.data)
        Y_flat.scatter_(0,
                        B_linear_indices.type(long_dtype).squeeze(),
                        B_flat.data)
        Y_flat.scatter_(0,
                        C_linear_indices.type(long_dtype).squeeze(),
                        C_flat.data)
        Y_flat.scatter_(0,
                        D_linear_indices.type(long_dtype).squeeze(),
                        D_flat.data)
...
class Model(nn.Module):
    def __init__(self, block1, block2, batch_size):
        ...
        self.proj_layer1 = self.make_proj_layer(block1, 64, d1=64, d2=256,
                                                stride=1)
        self.skip_layer1_1 = self.make_skip_layer(block1, 256, d1=64, d2=256,
                                                  stride=1)
        self.skip_layer1_2 = self.make_skip_layer(block1, 256, d1=64, d2=256,
                                                  stride=1)

        self.proj_layer2 = self.make_proj_layer(block1, 256, d1=128, d2=512,
                                                stride=2)
        ...
        self.proj_layer3 = self.make_proj_layer(block1, 512, d1=256, d2=1024,
                                                stride=2)
        ...
        self.proj_layer4 = self.make_proj_layer(block1, 1024, d1=512, d2=2048,
                                                stride=2)
        ...
        self.up_conv1 = self.make_up_conv_layer(block2, 1024, 512,
                                                self.batch_size)
        self.up_conv2 = self.make_up_conv_layer(block2, 512, 256,
                                                self.batch_size)
        self.up_conv3 = self.make_up_conv_layer(block2, 256, 128,
                                                self.batch_size)
        self.up_conv4 = self.make_up_conv_layer(block2, 128, 64,
                                                self.batch_size)

        self.conv3 = nn.Conv2d(64, 1, 3, padding=1)

        # Layers for Semantic Segmentation
        self.up_conv5 = self.make_up_conv_layer(block2, 128, 64,
                                                self.batch_size)
