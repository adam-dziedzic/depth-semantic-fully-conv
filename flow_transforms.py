...
class Compose(object):
    ...
    def __call__(self, input, target_depth, target_label):
        for i, t in enumerate(self.co_transforms):
            # print('After transform {}'.format(i))
            # print(np.max(target_label))
            input, target_depth, target_label = t(input, target_depth,
                                                  target_label)
...
class ArrayToTensor(object):
    ...
    def __call__(self, array):
        ...
        try:
            ...
            tensor = torch.from_numpy(np.expand_dims(array, axis=2)).permute(2,
                                                                             0,
                                                                             1)
...
class Scale(object):
    ...
    def __call__(self, inputs, target_depth=None, target_label=None):
        ...
        inputs = np.stack((ndimage.interpolation.zoom(inputs[:, :, 0], ratio,
                                                      order=self.order),
                           ndimage.interpolation.zoom(inputs[:, :, 1], ratio,
                                                      order=self.order), \
                           ndimage.interpolation.zoom(inputs[:, :, 2], ratio,
                                                      order=self.order)),
                          axis=2)

        if target_label is not None and target_depth is not None:

            target_label = ndimage.interpolation.zoom(target_label, ratio,
                                                      order=self.order)
            target_depth = ndimage.interpolation.zoom(target_depth, ratio,
                                                      order=self.order)
            return inputs, target_depth, target_label
...
class RandomCrop(object):
    ...
    def __call__(self, inputs, target_depth, target_label):
        ...
        return inputs, target_depth[y1: y1 + th, x1: x1 + tw], target_label[
                                                               y1: y1 + th,
                                                               x1: x1 + tw]
...
class RandomRotate(object):
    ...
    This is useful for dataAugmentation, especially for geometric problems such
    as FlowEstimation angle: max angle of the rotation
    interpolation order: Default: 2 (bilinear)
    reshape: Default: false. If set to true, image size will be set to keep
    every pixel in the image.
    diff_angle: Default: 0. Must stay less than 10 degrees, or linear
    approximation of flowmap will be off.
    """
    ...
    def __call__(self, inputs, target_depth, target_label):
        ...
        inputs = ndimage.interpolation.rotate(inputs, angle1,
                                              reshape=self.reshape,
                                              order=self.order)
        target_depth = ndimage.interpolation.rotate(target_depth, angle1,
                                                    reshape=self.reshape,
                                                    order=self.order)
        target_label = ndimage.interpolation.rotate(target_label, angle1,
                                                    reshape=self.reshape,
                                                    order=self.order)
...
class RandomCropRotate(object):
    ...
    def __call__(self, inputs, target):
        ...
        inputs[0] = ndimage.interpolation.rotate(inputs[0], angle1,
                                                 reshape=True, order=self.order)
        inputs[1] = ndimage.interpolation.rotate(inputs[1], angle2,
                                                 reshape=True, order=self.order)
        ...
        # keep angle1 and angle2 within [0,pi/2] with a reflection at pi/2:
        # -1rad is 1rad, 2rad is pi - 2 rad
        ...
        ratio = c_diag / max(c1 * c_diag + s1 * s_diag,
                             c2 * c_diag + s2 * s_diag)
